...
3. **Criar `contato@ultravis.ai`** — canal LGPD das páginas de Termos/Privacidade. **Caminho decidido 08/set: fazer na HOSTINGER, não na Cloudflare.** A investigação (5 runs do `criar-email-contato.yml`) terminou em achado, não em bloqueio: a zona `ultravis.ai` tem MX `mx1/mx2.hostinger.com` — **o e-mail do domínio já mora na Hostinger**. Habilitar Cloudflare Email Routing exigiria apagar esses MX, o que derrubaria qualquer e-mail `@ultravis.ai` em uso. Ação: criar a caixa/alias no painel da Hostinger (Emails → domínio `ultravis.ai`), ~2 min, sem tocar em DNS. **Não confundir com envio:** MX é só recebimento; o plano de SMTP pro alerta do vigia (cookbook §8) não é afetado.
   - Sub-achado útil da investigação: o `CLOUDFLARE_API_TOKEN` hoje tem `Email Routing Rules/Addresses: Edit` **e** `DNS: Edit` (o preflight do workflow confirma escopo por escopo) — serve pra operar DNS por API no futuro, se quisermos.

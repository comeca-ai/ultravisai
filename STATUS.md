...
3. **Criar `contato@ultravis.ai`** — canal LGPD das páginas de Termos/Privacidade. **Decidido 08/set: fazer na Cloudflare mesmo (Email Routing), trocando os MX.** A investigação (5 runs do `criar-email-contato.yml`) achou que a zona tinha MX `mx1/mx2.hostinger.com`; o dono confirmou que **não há nenhuma caixa em uso no `@ultravis.ai`**, então remover é seguro. O workflow ganhou o guard `ARMADO` (convenção do `limpar-workers-orfaos.yml`) e agora: imprime os MX atuais (isso É a receita de rollback) → apaga → habilita Email Routing → cria a regra `contato@ → jhonata.emerick@gmail.com`. **Se um dia voltar a existir e-mail no domínio, voltar `ARMADO` pra `'nao'` antes de qualquer re-run.**
   - **Não confundir com envio:** MX é só recebimento; o plano de SMTP pro alerta do vigia (cookbook §8) não é afetado.
   - Sub-achado útil: o `CLOUDFLARE_API_TOKEN` hoje tem `Email Routing Rules/Addresses: Edit` **e** `DNS: Edit` (o preflight do workflow confirma escopo por escopo) — serve pra operar DNS por API no futuro.

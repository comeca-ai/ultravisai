...
- [ ] **Proteger `/espelho` antes da fase B do espelho D1** — hoje a rota é pública e sem auth em `api.ultravis.ai`; com o banco vazio o dano é zero, com dados reais vira despejo de todas as orgs (D1 não tem RLS). Remédio: HTTP Basic no worker reusando `OPS_USER`/`OPS_PASS` (~15 linhas em `servirEspelho()`) + credencial no smoke do workflow. `P` · **bloqueia a fase B**
